%Cross Correlation (Same layout: fixed L)
...
for k = 1:5
    ...
    for j = 1:4096
        ...
        randomA = (((xAmax+xAmin)/2)+((xAmax - xAmin)/6)*randn);
            ...
            if i == 1
                fixedL = randomL;
                ppn1(j,:) = lambdaF*randomF + lambdaL*fixedL + lambdaM*randomM + lambdaO*randomO + lambdaA*randomA;
            else
                ppn2(j,:) = lambdaF*randomF + lambdaL*fixedL + lambdaM*randomM + lambdaO*randomO + lambdaA*randomA;
            end
        end
    end
...
xlabel('Index')
ylabel('Correlation')
title('Cross Correlation Gaussian Same Layout')
